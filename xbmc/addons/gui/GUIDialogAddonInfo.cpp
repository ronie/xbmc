  ...
  else
  {
    ...
    if (i != -1)
    {
      ...
      if (m_localAddon->Version() > versions[i].first)
        CServiceBroker::GetAddonMgr().AddRuleToList(m_localAddon->ID(),
      ...
      if ((m_localAddon->Version() < versions[i].first &&
        ...
        CServiceBroker::GetAddonMgr().RemoveRuleFromList(m_localAddon->ID(),
                                                         AddonUpdateRule::PIN_OLD_VERSION);
...
void CGUIDialogAddonInfo::OnToggleAutoUpdates()
{
  ...
  if (OnMessage(msg))
  {
    ...
    if (selected)
      CServiceBroker::GetAddonMgr().RemoveAllRulesFromList(m_localAddon->ID());
    else
      CServiceBroker::GetAddonMgr().AddRuleToList(m_localAddon->ID(),
